...
```mermaid
graph LR
    subgraph talkerNode ["talker"]
        pub["Publisher"]
    end
    topic(["/topic"])
    subgraph listenerNode ["listener"]
        sub["Subscriber"]
    end
    pub -->|"publish"| topic
    topic -->|"subscribe"| sub
...
1. Client が Request を送る（**Client はここで完了まで待機**）
...
これが「同期通信」の意味です．即座に結果が返るサービスには問題ありませんが，**処理に時間がかかるサービスを呼ぶと，少なくとも呼び出したスレッドはその間ほかの処理を進められません．**
