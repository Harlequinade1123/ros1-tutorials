# 13章: クラスを使った Kobuki プログラミング

12 章ではグローバル変数でコールバックと main 関数の間でデータを共有していました。ノードが複雑になるほどグローバル変数が増えてコードが読みにくくなります。10 章で学んだクラスを使うと整理できます。
...
| 章 | 内容 | 主な演習 |
|---|------|---------|
| 11 | 速度コマンド・往復動作 | 時間指定前進・往復繰り返し |
| 12 | バンパー・崖・オドメトリ | 接触停止・接触後退・距離移動 |
| 13 | クラス設計・LED/サウンド | 往復クラス・LED 表示・カウンター |

### 次に挑戦できること

- **パラメータで速度・距離を外から設定する**（8 章の内容を応用）
- **launch ファイルで Kobuki 起動から演習ノードまでまとめる**（5 章の応用）
