...
> この節は **Kobuki が接続されている場合**の内容です．

Kobuki を接続してドライバを起動すると，さまざまなデータを RViz で確認できます．
...
1. RViz を起動し，Fixed Frame を `odom` に設定
2. **[Add]** → **Odometry** → Topic を `/odom` に設定
3. **[Add]** → **Grid**（床のグリッドが表示されて見やすくなる）

Kobuki を手で動かすと，現在の向きと速度を示す矢印が更新されます．

### TF の表示

**[Add]** → **TF** を追加すると，座標フレーム間の関係が表示されます．

Kobuki が起動していれば，`odom` と `base_footprint`（ロボット底面）のフレームが矢印でつながって見えます．

### 軌跡（Path）を表示する

オドメトリの位置履歴を `nav_msgs/Path` として publish するノードを作ると，走行経路を線で描けます．

`~/catkin_ws/src/ros_tutorial/src/odom_path.cpp` を作成します．
...
void odomCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    g_path.header = msg->header;
    g_path.poses.push_back(pose);

    path_pub.publish(g_path);
}
...
int main(int argc, char **argv)
{
    ...
    path_pub = nh.advertise<nav_msgs::Path>("odom_path", 1);
...
target_link_libraries(odom_path ${catkin_LIBRARIES})
```

RViz では **[Add]** → **Path** → Topic を `/odom_path` に設定します．

Kobuki を走らせると，軌跡が線で描かれていきます．
