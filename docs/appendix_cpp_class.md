...
上の例（「最初の例」）では `main` の `}` を抜けた瞬間に自動で呼ばれ，`Hanako がシャットダウンしました` → `Taro がシャットダウンしました` と出力されます．
...
ROSのノードをクラスで書く場合，デストラクタはあまり明示的に書きません．`ros::NodeHandle` や `ros::Timer` などはクラス自身がデストラクタを持っており，オブジェクトが破棄されると自動でリソースを解放してくれるからです．  
ただし，`new` で確保したメモリや，外部デバイスとの接続を明示的に閉じたい場合にはデストラクタを書きます．
...
アクション通信（6章・14章）で出てくる，「関数を呼ぶための関数」を作る仕組みです．

`_1` は**プレースホルダー**で，「ここには呼び出し時に渡される引数が入る」を意味します．

**自由関数に追加引数を固定して渡す場合（6章）：**

```cpp
// executeCallback(goal, &server) という形で呼ばれる関数を作る
boost::bind(&executeCallback, _1, &server)
//                             ↑    ↑
//                          ゴール  固定する引数
```
...
```cpp
// this->callback(goal) という形で呼ばれる関数を作る
boost::bind(&MyClass::callback, this, _1)
//                               ↑    ↑
//                          オブジェクト  ゴール
```

メンバ関数の場合は「どのオブジェクトに対して呼ぶか」が必要なため，`this` を関数ポインタの直後に置きます．
