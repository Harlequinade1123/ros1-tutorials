...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        // 姿勢：進行方向を向く（yaw = t + 90°）
        tf2::Quaternion q;
        q.setRPY(0, 0, t + std::acos(-1.0) / 2.0);
...
`q.setRPY(0, 0, t + std::acos(-1.0)/2.0)` は「Z 軸まわりに `t + 90°` だけ回転」を意味します．`std::acos(-1.0)`は移植性のあるπの表現です．
...
既存の`find_package(catkin REQUIRED COMPONENTS ...)`へ次の3項目を追加します。5〜10章で追加した依存は削除しません：
