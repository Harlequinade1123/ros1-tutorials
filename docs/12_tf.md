...
```
world
 └── base_link  (tf: ロボットの現在位置，毎フレーム更新)
       └── camera_link  (tf: カメラの取り付け位置，固定)
```
...
| `q.setRPY(r, p, y)` | ロール・ピッチ・ヨーをクォータニオンに変換 |
...
| `lookupTransform(target, source, time)` | フレーム間の変換を取得 |
| `ros::Time(0)` | 最新の変換を取得 |
| `TransformException` | フレームがまだ存在しない場合などに発生 |
...
Kobuki ドライバを起動すると，オドメトリから自動的に `odom → base_footprint` の tf が配信されます．RViz に TF を表示すると，ロボットの動きに合わせてフレームが移動するのが確認できます（15章・16章の演習で体験できます）．
