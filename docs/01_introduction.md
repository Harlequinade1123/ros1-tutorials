...
```mermaid
graph LR
    camera_node["camera_node<br/>（Publisher）"] -->|"publish"| topic(["/camera/image<br/>（Topic）"])
    topic -->|"subscribe"| detection_node["detection_node<br/>（Subscriber）"]
```
...
```mermaid
sequenceDiagram
    client_node->>server_node: 目標（Goal）
    loop 処理中（何度も）
        server_node-->>client_node: フィードバック（進捗）
    end
    alt 正常完了
        server_node->>client_node: 結果（Result）
    else キャンセル
        client_node->>server_node: キャンセル（Cancel）
    end
```
...
graph TD
    ...
    NodeA["NodeA（Publisher）"]
    NodeB["NodeB"]
    NodeC["NodeC（Subscriber）"]
    topic(["/data"])
    NodeA -.->|"登録"| rosmaster
    NodeB -.->|"登録"| rosmaster
    NodeC -.->|"登録"| rosmaster
