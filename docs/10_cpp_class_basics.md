...
次の章では、ROS ノードをクラスで書き直します。この章では **クラスの基本** を純粋な C++ の話として説明します。
...
private:
    std::string name_;    // ロボットの名前
    int         speed_;   // 1歩あたりの移動距離
    int         distance_; // 累計移動距離
};

int main()
{
    // オブジェクトの生成（コンストラクタが呼ばれる）
...
このように **状態（count_）をクラスの中で管理** できるのがクラスの強みです。  
...
クラスの基本が分かったところで、次章ではこれを ROS ノードに活かします。

[→ 11章: クラスを使った ROS プログラミング](11_ros_with_class.md)
