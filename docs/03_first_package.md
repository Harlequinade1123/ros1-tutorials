...
> **補足: ビルド後の環境設定について**
>
> `catkin build` によってワークスペースの設定ファイル（`devel/setup.bash`）が生成・更新されます．
>
> 現在開いているターミナルで新しく追加したパッケージやノードを利用する場合は，必要に応じて以下を実行してワークスペースの設定を再読み込みしてください．
>
> ```bash
...
> ```
>
> を設定しているため，新しく開いたターミナルでは自動的にワークスペースの設定が読み込まれます．そのため，本チュートリアルでは以降，特に断りがない限り環境設定が読み込まれているものとして説明します．
