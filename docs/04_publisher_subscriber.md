...
> **【誤った理解】** ノードに「Publisherノード」「Subscriberノード」という種類がある

```mermaid
graph LR
    Pw["Publisherノード"] -->|"直接接続？"| Sw["Subscriberノード"]
```

> **【正しい理解】** ノードはトピックを介して通信する（トピックが仲介役）
...
```mermaid
graph TD
    A["talker が publish"] --> B["listener の受信キューに届く"]
    B --> C["ros::spin() がキューを処理"]
