# 18章: クラスを使った Kobuki プログラミング

17 章ではグローバル変数でコールバックと main 関数の間でデータを共有していました．ノードが複雑になるほどグローバル変数が増えてコードが読みにくくなります．14 章で学んだクラスを使うと整理できます．
...
| 章 | 内容 | 主な演習 |
|---|------|---------|
| 16 | 速度コマンド・往復動作 | 時間指定前進・往復繰り返し |
| 17 | バンパー・崖・オドメトリ | 接触停止・接触後退・距離移動 |
| 18 | クラス設計・LED/サウンド | 往復クラス・LED 表示・カウンター |
