# 12章: Kobuki センサー ── バンパー・崖・オドメトリ

> **注意**: この章の演習でロボットを動かすときは、**常に手でロボットを止められる準備**をしてください。バンパーに当たって停止するまで移動するため、壁までの距離を確認してから実行してください。旋回はケーブルが絡まるため使いません。
...
前進 → バンパー接触 → 後退 → 止まる、を **繰り返す** プログラムを書いてください。旋回はしません（ケーブルが絡まるため）。
...
> 最大 **0.1 m** 以下を指定してください。それ以上はスペースに収まらない可能性があります。
...
int main(int argc, char **argv)
{
    ...
    if (dist > 0.1)
    {
        ROS_WARN("%.3f m はスペースを超える可能性があります。0.1 m 以下を推奨します", dist);
    }
...
[→ 13章: クラスを使った Kobuki プログラミング](13_kobuki_class.md)
