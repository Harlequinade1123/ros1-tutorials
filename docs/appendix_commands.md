...
| `Ctrl + Z` | 実行中のプログラムを一時停止（サスペンド）．再開するには `fg`（フォアグラウンドに戻す）または `bg`（バックグラウンドで継続）|
| `Ctrl + L` | 画面をクリア（`clear` コマンドと同じ） |
| `history` | 過去のコマンド履歴を表示 |
| `<コマンド> --help` | コマンドの使い方・オプション一覧を表示（`-h` と書けるものも多い）|
...
| `&` | コマンドをバックグラウンドで実行する（ターミナルを占有しない）| `rosbag play data.bag &` |

---

## ROS コマンド

`rosnode`，`rostopic`，`rosservice` などの ROS コマンドはすべて `--help` オプションに対応しています．サブコマンドに対しても同様に使えます（例: `rostopic echo --help`）．
